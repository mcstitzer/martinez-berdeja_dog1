...
read SRRNUM <<< $(sed -n "${SLURM_ARRAY_TASK_ID}p" sra_lyrata.tosample.txt)
SAMPLENAME=$SRRNUM
#echo $SAMPLENAME
echo $SRRNUM
...
echo assembled_out_lyrata/${SRRNUM}.fna.contigs
if [ ! -f "assembled_out_lyrata/${SRRNUM}.fna.contigs" ]
then
...
if [ $SAMPLENAME!='' ]
then
mv ${SAMPLENAME}* assembled_out_lyrata
fi
fi
